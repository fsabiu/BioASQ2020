...
from utils.evaluate import evaluate_list
...
def encode_dataset(dataset_path, max_len, tokenizer, test_execution=-1):
    ...
    if(test_execution != -1):
        ...
        dataset_processed = load_data(
            dataset_path, "list", singleSnippets=True)[0:test_execution]
    else:
        dataset_processed = load_data(
            dataset_path, "list", singleSnippets=True)
    ...
    for sample in dataset_processed:
        ...
        if(len(input_ids) < max_len):
            ...
            for elem in sample[1]:
                answer_token = tokenizer.encode(elem[0])
...
def run_list_training(model, x_data, y_data, epochs, batch_size,logdir):
...
def test_list_model(trained_model, tokenizer, x_data_test, answer_list):
    print("...Evaluate")
    ...
    for i in range(len(x_data_test[0])):
        ...
        if(all_tokens[0:3]!=last_elem):#TODO:Fix this
            ...
            if(answer_list[i] not in merge_answer_list[last_elem_count-1]):
                merge_answer_list[last_elem_count-1].append(answer_list[i])
    ...
    evaluation=evaluate_list(predicted=predicted_cleaned, target=merge_answer_list)
    print(evaluation)
    return evaluation
...
def extract_answer(start_scores, end_scores, all_tokens,token_type_ids):
    ...
    results_array = KMaxCombinations(start_scores, end_scores, 1, token_type_ids)
...
def merge_answer(predicted):
    ...
    for key in res:
        ...
        final_predicted.append(
            sorted(complete_list, key=lambda t: t[1], reverse=True)[:10])
